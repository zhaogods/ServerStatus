...
def tupd():
    try:
        ...
        if sys.platform.startswith("linux"):
            ...
            t = len([c for c in conns if c.type == socket.SOCK_STREAM and c.status == 'ESTABLISHED'])
            u = len([c for c in conns if c.type == socket.SOCK_DGRAM])
        ...
        # 统一进程/线程统计（更稳定的实现）
        p = len(psutil.pids())
        d = sum([psutil.Process(pid).num_threads()
    ...
    while 1:
        try:
            ...
            while 1:
                ...
                Uptime = get_uptime()
                if hasattr(os, 'getloadavg') and (sys.platform.startswith('linux') or sys.platform == 'darwin'):
